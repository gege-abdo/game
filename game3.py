...
def play_game(string, speace,answer_level):
    replaced = []
    string = string.split()
    for word in string:
        ...
        if replacement != None:
            ...
            word = word.replace(replacement,is_right)
...
def choose (level):
    if level=="easy":
        print level1
        print play_game(level1, speace,answer_level1)
        return  right_answer(user_input,answer_level1)
    elif level=="medium":
        print level2
        print play_game(level2, speace,answer_level2)
        return right_answer(user_input,answer_level2)
    elif level=="hard":
        print level3
        print play_game(level3, speace,answer_level3)
        return right_answer(user_input,answer_level3)
    else:
        print ("choose between three options (easy-medium-hard):  " )
